...
class LocalDocQA:
    ...
    def doc_filter_by_score(self, docs_with_score):
        score_threshold = self.score_threshold
        docs_filter = []
        for i in range(len(docs_with_score)):
            if docs_with_score[i][1] <= score_threshold:
                docs_filter.append(docs_with_score[i])
        return docs_filter
    
    def find_ori_doc(self, docs_with_score):
        find_docs = []
        for i in range(len(docs_with_score)):
            row = docs_with_score[i][0].metadata['row']
    ...
    def get_knowledge_based_answer(self, query, vs_path, chat_history=[], streaming: bool = STREAMING):
        ...
        answer_result_stream_result = self.llm_model_chain(
            {"prompt": prompt, "history": [], "streaming": True})
        resp = ' '
    ...
    def extract_query_info(self, query):
        answer_result_result = self.llm_model_chain(
            {"prompt": query, "history": [], "streaming": False})
        return answer_result_result

    def process_answer(self, answer):
        query = generate_prompt_1(answer, PROMPT_TEMPLATE2)
        print(query)
        answer_result_result = self.llm_model_chain(
